...
// If readerDSN is empty, the reader uses the same connection as the writer.
//
// sqlDriver is the registered sql driver name. Pass "" to use the default "mysql" driver.
// When OpenTelemetry tracing is enabled, pass the name returned by otelsql.Register() instead —
